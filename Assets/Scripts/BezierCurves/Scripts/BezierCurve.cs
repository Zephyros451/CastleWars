...
public class BezierCurve : MonoBehaviour {
    ...
    public Vector3[] CalculateEvenlySpacedPoints(float spacing, float resolution = 1)
    {
        ...
        for (int i = 0; i < points.Length - 1; i++)
        {
            Vector3[] p = new Vector3[] { points[i].position, points[i].globalHandle2, points[i+1].globalHandle1, points[i+1].position };
            ...
            {
                t += 1f / divisions;
                Vector3 pointOnCurve = GetCubicCurvePoint(p[0], p[1], p[2], p[3], t);
